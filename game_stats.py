...
class GameStats:
    ...
    def stats_overview(self):
        ...
        for x in range(10):
            ...
            grid.addWidget(lbl1, x+1, 0)
            lbl2 = QLabel(str(round(self.p_data[x] / self.g_tot, 2)))
            lbl2.setAlignment(QtCore.Qt.AlignCenter)
            grid.addWidget(lbl2, x+1, 1)
            lb2 = QLabel(str(round(self.c_data[x] / self.g_tot, 2)))
            lbl2.setAlignment(QtCore.Qt.AlignCenter)
            grid.addWidget(lb2, x + 1, 2)
        for x in range(10, 17):
            ...
            lbl1.setAlignment(QtCore.Qt.AlignRight)
            grid.addWidget(lbl1, x + 1, 0)
            lb2 = QLabel(str(self.p_data[x]))
            lb2.setAlignment(QtCore.Qt.AlignCenter)
            grid.addWidget(lb2, x + 1, 1)
            lb3 = QLabel(str(self.c_data[x]))
            lb3.setAlignment(QtCore.Qt.AlignLeft)
            grid.addWidget(lb3, x + 1, 2)
        grid.addWidget(close_btn, 18, 1)
        close_btn.clicked.connect(win.close)
        win.show()
